...
const BTYPES = {
  ...
  running:   { label: 'Running',              bg: '#065F46', accent: '#6EE7B7' },
}
    ...
    <div style={S.overlay} onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={S.modal}>
        ...
        <input type="password" value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&go()}
          ...
          style={{...S.input, borderColor:err?ACCENT:'#3D1A2E', marginBottom:6}} />
...
function BlockModal({ block, onSave, onClose }) {
  ...
  const rtBtnStyle = (rt) => ({
    ...
    border: `1px solid ${resultType===rt ? (rt==='tiempo'?'#1D4ED8':rt==='kg'?'#F97316':'#86198F') : '#3D1A2E'}`,
    color: resultType===rt ? (rt==='tiempo'?'#93C5FD':rt==='kg'?'#FED7AA':'#F0ABFC') : '#6B3A55',
  })
  ...
  return (
    <div style={S.overlay} onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={S.modal}>
        ...
        {isWodType && (
          <div style={{marginTop:16,padding:'12px 14px',background:'#110810',border:'1px solid #2A1220',borderRadius:8}}>
            <label style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer',marginBottom: leaderboard ? 12 : 0}}>
              <div
                ...
                style={{
                  ...
                  background:leaderboard?ACCENT:'#2A1220',
              ...
              <span style={{fontSize:12,fontWeight:700,color:leaderboard?ACCENT:'#6B3A55'}}>
                Activar Leaderboard
              </span>
            </label>
...
export default function App() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#0E0A0D',color:'#F5D0E8',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',display:'flex',flexDirection:'column'}}>
        ...
        {isMobile ? (
          <>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
              <div>
                <div style={{fontSize:14,fontWeight:900,color:'#F5D0E8',letterSpacing:'0.08em'}}>⚡ RF PLANIFICACIÓN</div>
              ...
              <div style={{display:'flex',gap:6,alignItems:'center'}}>
                ...
                <button onClick={signOut} style={{background:'none',border:'1px solid #2A1220',borderRadius:8,color:'#3D1A2E',cursor:'pointer',fontSize:16,padding:'5px 10px'}}>↩</button>
              </div>
            </div>
          ...
          <div style={{display:'flex',alignItems:'center',gap:16}}>
            <div>
              <div style={{fontSize:15,fontWeight:900,color:'#F5D0E8',letterSpacing:'0.08em'}}>⚡ RF PLANIFICACIÓN</div>
            </div>
      ...
      <div style={{padding:'8px 14px',background:'#0C0C0C',borderBottom:'1px solid #120910',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
        <span style={{fontSize:11,color:'#3D1A2E'}}>
          ...
          {!isMobile && (
            <div style={{padding:'10px 16px',borderBottom:'1px solid #120910',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
              {isAdmin
                ...
                : (wdata.title ? <div style={{fontSize:16,fontWeight:700,color:'#F5D0E8'}}>{wdata.title}</div> : <div />)
              }
            </div>
          )}
          {isMobile && isAdmin && (
