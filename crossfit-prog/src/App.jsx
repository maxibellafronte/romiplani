...
const BTYPES = {
  warmup:    { label: 'Warm-up',              bg: '#4A7A79', accent: '#A8D5D4' },
  strength:  { label: 'Strength',             bg: '#3A6E99', accent: '#7EADDB' },
  barbell:   { label: 'Barbell Conditioning', bg: '#2A5A7A', accent: '#93C5E8' },
  metcon:    { label: 'Metcon',               bg: '#5A8A7A', accent: '#8ACABA' },
  wod:       { label: 'WOD',                  bg: '#3A7A78', accent: '#79BFBE' },
  skill:     { label: 'Skills',               bg: '#8A6A44', accent: '#E6D4BE' },
  accessory: { label: 'Accessory',            bg: '#7A5A34', accent: '#D4BC9E' },
  running:   { label: 'Running',              bg: '#3A6A4A', accent: '#8ACAAA' },
}
...
const S = {
  ...
  modal: { background:'#0E1614', border:'1px solid #2A4A44', borderRadius:12, padding:22, width:'min(440px,94vw)', maxHeight:'90vh', overflowY:'auto' },
  label: { fontSize:10, color:'#4A7A70', textTransform:'uppercase', letterSpacing:'0.08em', display:'block', marginTop:14 },
  input: { display:'block', width:'100%', background:'#0E1210', border:'1px solid #2A4A44', borderRadius:8, color:'#D4EDE8', padding:'10px 12px', fontSize:13, outline:'none', marginTop:5 },
  btnGhost: { padding:'10px 16px', background:'none', border:'1px solid #2A4A44', borderRadius:8, color:'#4A7A70', cursor:'pointer', fontSize:13 },
    ...
    <div style={S.overlay} onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={S.modal}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
          <span style={{fontWeight:700,fontSize:14,color:'#D4EDE8'}}>Acceso Coach</span>
          <button onClick={onClose} style={{background:'none',border:'none',color:'#4A7A70',fontSize:22,cursor:'pointer'}}>×</button>
        </div>
        <div style={{textAlign:'center',marginBottom:18}}>
          <div style={{fontSize:34,marginBottom:8}}>🔐</div>
          <p style={{fontSize:12,color:'#4A7A70'}}>Ingresá la contraseña de administrador</p>
        </div>
        <input type="password" value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&go()}
          placeholder="Contraseña..." autoFocus
          style={{...S.input, borderColor:err?ACCENT:'#2A4A44', marginBottom:6}} />
...
function BlockModal({ block, onSave, onClose }) {
  ...
  const [leaderboard, setLeaderboard] = useState(block?.leaderboard || false)
  const [resultType, setResultType] = useState(block?.resultType || 'tiempo')

  const isWodType = ['wod','metcon'].includes(type)

  const rtBtnStyle = (rt) => ({
    flex:1, padding:'7px 0', fontSize:11, fontWeight:700, cursor:'pointer', borderRadius:6,
    background: resultType===rt ? (rt==='tiempo'?'#1D4ED815':rt==='kg'?'#D9770615':'#86198F15') : 'none',
    border: `1px solid ${resultType===rt ? (rt==='tiempo'?'#1D4ED8':rt==='kg'?'#F97316':'#86198F') : '#2A4A44'}`,
    color: resultType===rt ? (rt==='tiempo'?'#93C5FD':rt==='kg'?'#FED7AA':'#F0ABFC') : '#4A7A70',
  })
  ...
  return (
    <div style={S.overlay} onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={S.modal}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}>
          <span style={{fontWeight:700,fontSize:14,color:'#D4EDE8'}}>{block?.id ? 'Editar bloque' : 'Nuevo bloque'}</span>
          <button onClick={onClose} style={{background:'none',border:'none',color:'#4A7A70',fontSize:22,cursor:'pointer'}}>×</button>
        </div>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:7}}>
          {Object.entries(BTYPES).map(([k,v]) => (
            <button key={k} onClick={()=>setType(k)} style={{
              ...
              border:`1px solid ${k===type ? v.bg : '#2A4A44'}`,
              background: k===type ? v.bg+'40' : 'none',
              color: k===type ? v.accent : '#4A7A70',
            }}>{v.label}</button>
          ))}
        </div>
        ...
        {/* Leaderboard toggle — solo para WOD y Metcon */}
        {isWodType && (
          <div style={{marginTop:16,padding:'12px 14px',background:'#0E1210',border:'1px solid #1E3530',borderRadius:8}}>
            <label style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer',marginBottom: leaderboard ? 12 : 0}}>
              <div
                onClick={()=>setLeaderboard(l=>!l)}
                style={{
                  width:38,height:22,borderRadius:11,
                  background:leaderboard?ACCENT:'#1E3530',
                  position:'relative',transition:'background 0.2s',cursor:'pointer',flexShrink:0
                }}
              >
                <div style={{
                  position:'absolute',top:3,left:leaderboard?18:3,width:16,height:16,
                  borderRadius:'50%',background:'white',transition:'left 0.2s'
                }}/>
              </div>
              <span style={{fontSize:12,fontWeight:700,color:leaderboard?ACCENT:'#4A7A70'}}>
                Activar Leaderboard
              </span>
            </label>
            ...
            {leaderboard && (
              <>
                <span style={{...S.label,marginTop:8}}>Tipo de resultado</span>
                <div style={{display:'flex',gap:6,marginTop:7}}>
                  <button onClick={()=>setResultType('tiempo')} style={rtBtnStyle('tiempo')}>⏱ Tiempo</button>
                  <button onClick={()=>setResultType('reps')}   style={rtBtnStyle('reps')}>💪 Reps</button>
                  <button onClick={()=>setResultType('kg')}     style={rtBtnStyle('kg')}>⚖️ Kg</button>
                </div>
              </>
            )}
          </div>
    ...
    <div style={{background:t.bg+'18', border:`1px solid ${t.bg}44`, borderRadius:8, marginBottom:8, overflow:'hidden'}}>
      <div onClick={()=>setOpen(o=>!o)}
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,minWidth:0}}>
          ...
          {block.title && !open && (
            <span style={{fontSize:12,color:'#7AADA0',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>— {block.title}</span>
          )}
        </div>
          ...
          {isAdmin && (
            <>
              <button onClick={e=>{e.stopPropagation();onEdit(block)}}
                style={{background:'none',border:'none',color:'#5A8A80',cursor:'pointer',fontSize:12,padding:'2px 5px'}}>✏</button>
              <button onClick={e=>{e.stopPropagation();onDel(block.id)}}
                style={{background:'none',border:'none',color:'#3A6058',cursor:'pointer',fontSize:15,padding:'2px 5px'}}>×</button>
            </>
          )}
          <span style={{fontSize:11,color:'#3A6058',transition:'transform 0.2s',display:'inline-block',transform:open?'rotate(180deg)':'rotate(0deg)'}}>▾</span>
        </div>
      </div>
      {open && (
        <div style={{padding:isMobile?'0 12px 12px':'0 10px 10px',borderTop:`1px solid ${t.bg}30`}}>
          {block.title && <div style={{fontSize:isMobile?14:13,fontWeight:700,color:'#D4EDE8',marginTop:10,marginBottom:block.content?6:0}}>{block.title}</div>}
          {block.content && <div style={{fontSize:isMobile?13:11,color:'#7AADA0',whiteSpace:'pre-wrap',lineHeight:1.6,wordBreak:'break-word'}}>{block.content}</div>}
          {hasLeaderboard && (
            <WodLeaderboard
              blockId={block.id}
              resultType={block.resultType || 'tiempo'}
              userId={userId}
              userName={userName}
              isAdmin={isAdmin}
            />
...
function DayCard({ dayKey, label, date, dateObj, data, isAdmin, isToday, isMobile, defaultOpen, userId, userName, onUpdate, onAdd, onEdit, onDel }) {
  ...
  return (
    <div style={{
      background:isToday?'#0D1714':'#0E1210',
      border:`1px solid ${isToday?ACCENT:'#1E3530'}`,
      ...
      <div onClick={()=>setOpen(o=>!o)} style={{
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,minWidth:0}}>
          ...
          {data.focus && !open && <span style={{fontSize:10,color:'#3A6058',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>· {data.focus}</span>}
        </div>
        <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
          <span style={{fontSize:isMobile?12:10,color:'#3A6058'}}>{isMobile?fmtDateLong(dateObj):date}</span>
          <span style={{fontSize:12,color:'#3A6058',transition:'transform 0.2s',display:'inline-block',transform:open?'rotate(180deg)':'rotate(0deg)'}}>▾</span>
        </div>
      </div>
      {open && (
        <div style={{padding:isMobile?'10px 14px':'9px 10px',borderTop:`1px solid ${isToday?ACCENT+'30':'#1A2420'}`}}>
          {isAdmin ? (
            <input value={data.focus||''} onChange={e=>onUpdate({...data,focus:e.target.value})}
              placeholder="foco del día..."
              style={{background:'none',border:'none',borderBottom:'1px solid #2A1220',color:'#7AADA0',fontSize:10,
                width:'100%',marginBottom:10,padding:'3px 0',outline:'none',textTransform:'uppercase',letterSpacing:'0.06em'}}
              onClick={e=>e.stopPropagation()} />
          ) : data.focus ? (
            <div style={{fontSize:10,color:'#5A8A80',marginBottom:8,textTransform:'uppercase',letterSpacing:'0.07em',fontWeight:600}}>{data.focus}</div>
          ) : null}
          {isAdmin && (
            <label style={{display:'flex',alignItems:'center',gap:6,marginBottom:10,cursor:'pointer'}} onClick={e=>e.stopPropagation()}>
              ...
              <span style={{fontSize:10,color:'#3A6058',textTransform:'uppercase',letterSpacing:'0.07em'}}>Día de descanso</span>
            </label>
          )}
          {data.isRest ? (
            <div style={{textAlign:'center',color:'#2A4A44',fontSize:11,padding:'16px 0',letterSpacing:'0.15em',fontWeight:600}}>— DESCANSO —</div>
          ) : (
            <>
              {(data.blocks||[]).map(b => (
              ...
              {isAdmin && (
                <button onClick={e=>{e.stopPropagation();onAdd()}}
                  style={{width:'100%',background:'none',border:'1px dashed #3D1A2E',borderRadius:6,
                    color:'#3A6058',fontSize:11,padding:isMobile?'10px 0':'7px 0',
...
export default function App() {
  ...
  const [adminTab, setAdminTab] = useState('programming')
  ...
  const persist = (data) => {
    ...
    saveTimer.current = setTimeout(async () => {
      try { await saveWeek(year, week, data, 'General') }
  ...
  if (authLoading) return (
    <div style={{minHeight:'100vh',background:'#0F1412',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{fontSize:12,color:'#2A4A44',letterSpacing:'0.12em'}}>CARGANDO...</div>
    </div>
  )
  ...
  if (profile?.status === 'inactive') return (
    <div style={{minHeight:'100vh',background:'#0F1412',display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <div style={{background:'#0E1614',border:'1px solid #2A4A44',borderRadius:14,padding:36,width:'min(420px,100%)',textAlign:'center'}}>
        ...
        <h2 style={{color:'#D4EDE8',fontSize:18,fontWeight:800,marginBottom:12}}>Cuenta inactiva</h2>
        <p style={{color:'#7AADA0',fontSize:14,lineHeight:1.6,marginBottom:24}}>Tu acceso fue desactivado. Contactá al coach para más información.</p>
        <button onClick={signOut} style={{background:'none',border:'1px solid #2A4A44',borderRadius:8,color:'#4A7A70',cursor:'pointer',fontSize:13,padding:'10px 24px'}}>Cerrar sesión</button>
      </div>
    </div>
  )

  return (
    <div style={{minHeight:'100vh',background:'#0F1412',color:'#D4EDE8',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',display:'flex',flexDirection:'column'}}>
      ...
      {/* Header */}
      <div style={{background:'#0A0F0E',borderBottom:'1px solid #1E0A17',padding:isMobile?'10px 14px':'11px 16px'}}>
        {isMobile ? (
          <>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
              <div>
                <div style={{fontSize:14,fontWeight:900,color:'#D4EDE8',letterSpacing:'0.08em'}}>⚡ CF PROGRAMACIÓN</div>
              ...
              <div style={{display:'flex',gap:6,alignItems:'center'}}>
                {isAdmin
                  ...
                  : <button onClick={()=>setShowLogin(true)} style={{background:'none',border:'1px solid #2A4A44',borderRadius:8,color:'#4A7A70',cursor:'pointer',fontSize:10,padding:'7px 13px'}}>COACH</button>
                }
                <button onClick={signOut} style={{background:'none',border:'1px solid #1E3530',borderRadius:8,color:'#2A4A44',cursor:'pointer',fontSize:16,padding:'5px 10px'}}>↩</button>
              </div>
            </div>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <button onClick={()=>nav(-1)} style={{background:'none',border:'1px solid #1E3530',borderRadius:6,color:'#4A7A70',cursor:'pointer',width:36,height:36,fontSize:18,display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button>
              <div style={{textAlign:'center',flex:1}}>
                <div style={{fontSize:12,fontWeight:800,color:ACCENT,letterSpacing:'0.1em'}}>SEMANA {String(week).padStart(2,'0')}</div>
                <div style={{fontSize:10,color:'#3A6058'}}>{fmtDate(monday)} — {fmtDate(sunday)} · {year}</div>
              </div>
              <button onClick={()=>nav(1)} style={{background:'none',border:'1px solid #1E3530',borderRadius:6,color:'#4A7A70',cursor:'pointer',width:36,height:36,fontSize:18,display:'flex',alignItems:'center',justifyContent:'center'}}>›</button>
          ...
          <div style={{display:'flex',alignItems:'center',gap:16}}>
            <div>
              <div style={{fontSize:15,fontWeight:900,color:'#D4EDE8',letterSpacing:'0.08em'}}>⚡ CF PROGRAMACIÓN</div>
              <div style={{fontSize:9,color:'#2A4A44',textTransform:'uppercase',letterSpacing:'0.12em'}}>Box Programming</div>
            </div>
            <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:12}}>
              <button onClick={()=>nav(-1)} style={{background:'none',border:'1px solid #1A2420',borderRadius:4,color:'#3A6058',cursor:'pointer',width:28,height:28,fontSize:16,display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button>
              <div style={{textAlign:'center'}}>
                <div style={{fontSize:13,fontWeight:800,color:ACCENT,letterSpacing:'0.1em'}}>SEMANA {String(week).padStart(2,'0')}</div>
                <div style={{fontSize:9,color:'#2A4A44'}}>{fmtDate(monday)} — {fmtDate(sunday)} · {year}</div>
              </div>
              <button onClick={()=>nav(1)} style={{background:'none',border:'1px solid #1A2420',borderRadius:4,color:'#3A6058',cursor:'pointer',width:28,height:28,fontSize:16,display:'flex',alignItems:'center',justifyContent:'center'}}>›</button>
            ...
            <div style={{display:'flex',alignItems:'center',gap:8}}>
              {saving && <span style={{fontSize:9,color:'#2A4A44'}}>GUARDANDO...</span>}
              {isAdmin
                ...
                : <button onClick={()=>setShowLogin(true)} style={{background:'none',border:'1px solid #1A2420',borderRadius:6,color:'#2A4A44',cursor:'pointer',fontSize:10,padding:'6px 12px'}}>COACH</button>
              }
              <button onClick={signOut} title="Cerrar sesión" style={{background:'none',border:'1px solid #1A2420',borderRadius:6,color:'#2A4A44',cursor:'pointer',fontSize:10,padding:'6px 10px'}}>↩</button>
            </div>
          </div>
        )}
      </div>

      {/* Barra usuario / tabs admin */}
      <div style={{padding:'8px 14px',background:'#0C100F',borderBottom:'1px solid #120910',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
        <span style={{fontSize:11,color:'#2A4A44'}}>
          {isAdmin ? '⚡ Modo Coach' : userName}
        </span>
        {isAdmin && (
          <div style={{display:'flex',gap:6}}>
            <button onClick={()=>setAdminTab('programming')} style={{padding:'4px 12px',fontSize:11,fontWeight:600,cursor:'pointer',borderRadius:6,background:adminTab==='programming'?'#2A4A4422':'none',border:`1px solid ${adminTab==='programming'?'#2A4A44':'#1E3530'}`,color:adminTab==='programming'?'#A8D5D4':'#3A6058'}}>📅 Programación</button>
            <button onClick={()=>setAdminTab('athletes')} style={{padding:'4px 12px',fontSize:11,fontWeight:600,cursor:'pointer',borderRadius:6,background:adminTab==='athletes'?'#2A4A4422':'none',border:`1px solid ${adminTab==='athletes'?'#2A4A44':'#1E3530'}`,color:adminTab==='athletes'?'#A8D5D4':'#3A6058'}}>👥 Atletas</button>
            <button onClick={()=>setAdminTab('activity')} style={{padding:'4px 12px',fontSize:11,fontWeight:600,cursor:'pointer',borderRadius:6,background:adminTab==='activity'?'#2A4A4422':'none',border:`1px solid ${adminTab==='activity'?'#2A4A44':'#1E3530'}`,color:adminTab==='activity'?'#A8D5D4':'#3A6058'}}>📈 Actividad</button>
          </div>
        )}
      </div>
      ...
      {isAdmin && adminTab === 'athletes' ? (
        ...
        <>
            ...
            <div style={{padding:'10px 16px',borderBottom:'1px solid #120910',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
              {isAdmin
                ? <input value={wdata.title||''} onChange={e=>updTitle(e.target.value)} placeholder="Título de la semana..."
                    style={{background:'none',border:'none',color:'#D4EDE8',fontSize:16,fontWeight:700,flex:1,outline:'none',letterSpacing:'0.02em'}} />
                : <div style={{fontSize:16,fontWeight:700,color:wdata.title?'#D4EDE8':'#1E3530'}}>{wdata.title||'SIN TÍTULO'}</div>
              }
              <span style={{fontSize:9,color:'#1E3530',flexShrink:0}}>{totalBlocks} BLOQUES</span>
            </div>
          )}
          {isMobile && isAdmin && (
            <div style={{padding:'8px 14px',borderBottom:'1px solid #120910'}}>
              <input value={wdata.title||''} onChange={e=>updTitle(e.target.value)} placeholder="Título de la semana..."
                style={{background:'none',border:'none',color:'#D4EDE8',fontSize:14,fontWeight:700,width:'100%',outline:'none'}} />
            </div>
          )}
          ...
          <div style={{flex:1,padding:'8px 10px',overflowX:isMobile?'visible':'auto'}}>
            {loading ? (
              <div style={{textAlign:'center',padding:60,color:'#2A4A44',letterSpacing:'0.12em',fontSize:12}}>CARGANDO PROGRAMACIÓN...</div>
            ) : error ? (
