...
const S = {
  ...
  modal: { background:'#EEF2F0', border:'1px solid #C4CDD4', borderRadius:12, padding:22, width:'min(440px,94vw)', maxHeight:'90vh', overflowY:'auto', boxShadow:'0 8px 28px rgba(31,58,74,0.18)' },
...
function BlockModal({ block, onSave, onClose }) {
  ...
  const rtBtnStyle = (rt) => ({
    ...
    border: `1px solid ${resultType===rt ? (rt==='tiempo'?'#31708E':rt==='kg'?'#B5761E':'#5085A5') : '#AEB9C0'}`,
  ...
  return (
    <div style={S.overlay} onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={S.modal}>
        ...
        <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:7}}>
          {Object.entries(BTYPES).map(([k,v]) => (
            <button key={k} onClick={()=>setType(k)} style={{
              ...
              border:`1px solid ${k===type ? v.bg : '#AEB9C0'}`,
        ...
        {isWodType && (
          <div style={{marginTop:16,padding:'12px 14px',background:'#E2E8EA',border:'1px solid #C4CDD4',borderRadius:8}}>
            <label style={{display:'flex',alignItems:'center',gap:10,cursor:'pointer',marginBottom: leaderboard ? 12 : 0}}>
              <div
                ...
                style={{
                  ...
                  background:leaderboard?ACCENT:'#C4CDD4',
    ...
    <div style={{background:t.bg+'18', border:`1px solid ${t.bg}44`, borderRadius:8, marginBottom:8, overflow:'hidden'}}>
      <div onClick={()=>setOpen(o=>!o)}
        ...
        <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
          ...
          <span style={{fontSize:11,color:'#8A98A2',transition:'transform 0.2s',display:'inline-block',transform:open?'rotate(180deg)':'rotate(0deg)'}}>▾</span>
        </div>
      </div>
      {open && (
...
function DayCard({ dayKey, label, date, dateObj, data, isAdmin, isToday, isMobile, defaultOpen, userId, userName, onUpdate, onAdd, onEdit, onDel }) {
  ...
  return (
    <div style={{
      ...
      border:`1px solid ${isToday?ACCENT:'#C4CDD4'}`,
      ...
      <div onClick={()=>setOpen(o=>!o)} style={{
        ...
        <div style={{display:'flex',alignItems:'center',gap:8,flexShrink:0}}>
          ...
          <span style={{fontSize:12,color:'#8A98A2',transition:'transform 0.2s',display:'inline-block',transform:open?'rotate(180deg)':'rotate(0deg)'}}>▾</span>
        </div>
      </div>
      {open && (
        <div style={{padding:isMobile?'10px 14px':'9px 10px',borderTop:`1px solid ${isToday?ACCENT+'30':'#C4CDD4'}`}}>
          ...
          {isAdmin && (
            <label style={{display:'flex',alignItems:'center',gap:6,marginBottom:10,cursor:'pointer'}} onClick={e=>e.stopPropagation()}>
              ...
              <span style={{fontSize:10,color:'#8A98A2',textTransform:'uppercase',letterSpacing:'0.07em'}}>Día de descanso</span>
            </label>
          )}
          {data.isRest ? (
            ...
            <>
              ...
              {isAdmin && (
                <button onClick={e=>{e.stopPropagation();onAdd()}}
                  style={{width:'100%',background:'none',border:'1px dashed #AEB9C0',borderRadius:6,
                    color:'#8A98A2',fontSize:11,padding:isMobile?'10px 0':'7px 0',
...
export default function App() {
  ...
  if (authLoading) return (
    <div style={{minHeight:'100vh',background:'#DCE3E8',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <div style={{fontSize:12,color:'#AEB9C0',letterSpacing:'0.12em'}}>CARGANDO...</div>
    </div>
  )
  ...
  if (profile?.status === 'inactive') return (
    <div style={{minHeight:'100vh',background:'#DCE3E8',display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <div style={{background:'#EEF2F0',border:'1px solid #AEB9C0',borderRadius:14,padding:36,width:'min(420px,100%)',textAlign:'center'}}>
        ...
        <button onClick={signOut} style={{background:'none',border:'1px solid #AEB9C0',borderRadius:8,color:'#7A8FA0',cursor:'pointer',fontSize:13,padding:'10px 24px'}}>Cerrar sesión</button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{minHeight:'100vh',background:'#DCE3E8',color:'#1F3A4A',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',display:'flex',flexDirection:'column'}}>
      ...
      {/* Header */}
      <div style={{background:'#EEF2F0',borderBottom:'1px solid #C4CDD4',padding:isMobile?'10px 14px':'11px 16px'}}>
        {isMobile ? (
          <>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <button onClick={()=>nav(-1)} style={{background:'none',border:'1px solid #C4CDD4',borderRadius:6,color:'#7A8FA0',cursor:'pointer',width:36,height:36,fontSize:18,display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button>
              <div style={{textAlign:'center',flex:1}}>
                <div style={{fontSize:12,fontWeight:800,color:ACCENT,letterSpacing:'0.1em'}}>SEMANA {String(week).padStart(2,'0')}</div>
                <div style={{fontSize:10,color:'#8A98A2'}}>{fmtDate(monday)} — {fmtDate(sunday)} · {year}</div>
              </div>
              <button onClick={()=>nav(1)} style={{background:'none',border:'1px solid #C4CDD4',borderRadius:6,color:'#7A8FA0',cursor:'pointer',width:36,height:36,fontSize:18,display:'flex',alignItems:'center',justifyContent:'center'}}>›</button>
          ...
          <div style={{display:'flex',alignItems:'center',gap:16}}>
            ...
            <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:12}}>
              <button onClick={()=>nav(-1)} style={{background:'none',border:'1px solid #C4CDD4',borderRadius:4,color:'#8A98A2',cursor:'pointer',width:28,height:28,fontSize:16,display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button>
              <div style={{textAlign:'center'}}>
                <div style={{fontSize:13,fontWeight:800,color:ACCENT,letterSpacing:'0.1em'}}>SEMANA {String(week).padStart(2,'0')}</div>
                <div style={{fontSize:9,color:'#AEB9C0'}}>{fmtDate(monday)} — {fmtDate(sunday)} · {year}</div>
              </div>
              <button onClick={()=>nav(1)} style={{background:'none',border:'1px solid #C4CDD4',borderRadius:4,color:'#8A98A2',cursor:'pointer',width:28,height:28,fontSize:16,display:'flex',alignItems:'center',justifyContent:'center'}}>›</button>
      ...
      <div style={{padding:'8px 14px',background:'#E2E8EA',borderBottom:'1px solid #EEF2F0',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
      ...
      {isAdmin && adminTab === 'athletes' ? (
        ...
        <>
          {/* Título semana */}
          {!isMobile && (
            <div style={{padding:'10px 16px',borderBottom:'1px solid #EEF2F0',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
              {isAdmin
          ...
          {isMobile && isAdmin && (
            <div style={{padding:'8px 14px',borderBottom:'1px solid #EEF2F0'}}>
              <input value={wdata.title||''} onChange={e=>updTitle(e.target.value)} placeholder="Título de la semana..."
          ...
          <div style={{flex:1,padding:'8px 10px',overflowX:isMobile?'visible':'auto'}}>
            {loading ? (
              <div style={{textAlign:'center',padding:60,color:'#AEB9C0',letterSpacing:'0.12em',fontSize:12}}>CARGANDO PROGRAMACIÓN...</div>
            ) : error ? (
          ...
          {/* Footer */}
          <div style={{padding:'8px 14px',borderTop:'1px solid #EEF2F0',display:'flex',flexWrap:'wrap',justifyContent:'space-between',alignItems:'center',gap:6}}>
