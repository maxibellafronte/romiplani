...
export default function ActivityPanel() {
  ...
  const tabStyle = (t) => ({
    ...
    background: view === t ? '#E8F0F5' : 'none',
    border: `1px solid ${view === t ? ACCENT : '#DDE5EC'}`,
    color: view === t ? ACCENT : '#90A4B4',
  })

  const rangeBtnStyle = (d) => ({
    padding: '4px 10px', fontSize: 10, fontWeight: 600, cursor: 'pointer', borderRadius: 5,
    background: dateRange === d ? '#E8F0F5' : 'none',
    border: `1px solid ${dateRange === d ? '#C0CDD8' : '#DDE5EC'}`,
    color: dateRange === d ? '#245166' : '#90A4B4',
  })

  if (loading) return (
    <div style={{padding:40,textAlign:'center',color:'#B0BCC6',letterSpacing:'0.1em',fontSize:12}}>CARGANDO ACTIVIDAD...</div>
  )
  if (error) return (
    ...
    <div style={{padding:16}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:10}}>
        ...
        <div style={{display:'flex',gap:6,alignItems:'center'}}>
          <span style={{fontSize:10,color:'#90A4B4'}}>Últimos:</span>
          {[7,15,30,60].map(d => (
            <button key={d} onClick={()=>setDateRange(d)} style={rangeBtnStyle(d)}>{d}d</button>
          ))}
          <button onClick={load} style={{background:'none',border:'1px solid #DDE5EC',borderRadius:6,color:'#90A4B4',cursor:'pointer',fontSize:11,padding:'4px 10px',marginLeft:4}}>↻</button>
        </div>
      </div>
      ...
      <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
        {[
          ...
          <div key={s.label} style={{flex:1,minWidth:100,background:'#FFFFFF',border:'1px solid #DDE5EC',borderRadius:8,padding:'10px 12px',textAlign:'center'}}>
            <div style={{fontSize:20,fontWeight:900,color:ACCENT}}>{s.value}</div>
            <div style={{fontSize:9,color:'#90A4B4',textTransform:'uppercase',letterSpacing:'0.07em',marginTop:3}}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      {view === 'summary' && (
        <div>
          {sortedDays.length === 0 ? (
            <div style={{textAlign:'center',color:'#B0BCC6',padding:'30px 0',fontSize:12,letterSpacing:'0.1em'}}>— SIN ACTIVIDAD EN ESTE PERÍODO —</div>
          ) : sortedDays.map(day => {
            const dayLogs = byDay[day]
            const uniqueUsers = [...new Set(dayLogs.map(l => l.user_id))].length
            return (
              <div key={day} style={{background:'#FFFFFF',border:'1px solid #DDE5EC',borderRadius:8,padding:'10px 14px',marginBottom:8}}>
                  ...
                  {[...new Set(dayLogs.map(l=>l.user_id))].map(uid => {
                    ...
                    return (
                      <div key={uid}
                        ...
                        style={{background:'#F0F4F8',border:'1px solid #DDE5EC',borderRadius:5,padding:'3px 9px',cursor:'pointer'}}>
                        <span style={{fontSize:11,color:'#5A7286'}}>{name}</span>
                        {userLogs.length > 1 && <span style={{fontSize:10,color:'#90A4B4',marginLeft:5}}>×{userLogs.length}</span>}
                      </div>
                    )
                  })}
                </div>
        ...
        <div>
          {sortedUsers.length === 0 ? (
            ...
            return (
              <div key={uid}
                onClick={()=>{setSelectedUser(uid);setView('detail')}}
                style={{background:'#FFFFFF',border:'1px solid #DDE5EC',borderRadius:8,padding:'11px 14px',marginBottom:8,cursor:'pointer',display:'flex',justifyContent:'space-between',alignItems:'center',gap:12}}>
                <div style={{minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:700,color:'#1F3A4A',marginBottom:2}}>{data.user_name || data.email}</div>
                  <div style={{fontSize:11,color:'#90A4B4'}}>{data.email}</div>
                </div>
                <div style={{display:'flex',gap:16,flexShrink:0,textAlign:'center'}}>
                  <div>
                    ...
                    <div style={{fontSize:9,color:'#B0BCC6',textTransform:'uppercase',letterSpacing:'0.06em'}}>accesos</div>
                  </div>
                  <div>
                    <div style={{fontSize:16,fontWeight:800,color:'#5A7286'}}>{daysActive}</div>
                    <div style={{fontSize:9,color:'#B0BCC6',textTransform:'uppercase',letterSpacing:'0.06em'}}>días</div>
                  </div>
                  <div style={{textAlign:'right'}}>
                    <div style={{fontSize:11,color:'#7A8FA0'}}>{fmtDate(lastLog.created_at)}</div>
                    <div style={{fontSize:9,color:'#B0BCC6'}}>último acceso</div>
                  </div>
                </div>
              </div>
            )
      ...
      {view === 'detail' && userDetail && (
        <div>
          <div style={{background:'#FFFFFF',border:'1px solid #C0CDD8',borderRadius:8,padding:'12px 14px',marginBottom:14}}>
            <div style={{fontSize:14,fontWeight:700,color:'#1F3A4A',marginBottom:2}}>{userDetail.user_name || userDetail.email}</div>
            <div style={{fontSize:11,color:'#90A4B4'}}>{userDetail.email}</div>
          ...
          {Object.entries(
            ...
            <div key={day} style={{background:'#FFFFFF',border:'1px solid #DDE5EC',borderRadius:8,padding:'10px 14px',marginBottom:8}}>
              ...
              {dayLogs.length > 1 && (
                <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                  {dayLogs.map(log=>(
                    <span key={log.id} style={{fontSize:11,color:'#7A8FA0',background:'#F0F4F8',padding:'2px 8px',borderRadius:4}}>{fmtTime(log.created_at)}</span>
                  ))}
                </div>
              )}
            </div>
