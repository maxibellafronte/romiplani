...
export default function AthletesPanel() {
  ...
  const filterBtnStyle = (f) => ({
    ...
    border: `1px solid ${filter === f ? ACCENT : '#2A4A44'}`,
    borderRadius: 6, color: filter === f ? ACCENT : '#4A7A70',
    cursor: 'pointer', fontSize: 11, fontWeight: 600,
  })

  if (loading) return (
    <div style={{padding:40,textAlign:'center',color:'#2A4A44',letterSpacing:'0.1em',fontSize:12}}>
      CARGANDO ATLETAS...
    </div>
  )
  ...
  return (
    <div style={{padding:16}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        ...
        <button onClick={load} style={{background:'none',border:'1px solid #3D1A2E',borderRadius:6,color:'#4A7A70',cursor:'pointer',fontSize:11,padding:'5px 10px'}}>
          ↻ Actualizar
        </button>
      </div>
      ...
      {/* Lista */}
      {!filtered.length ? (
        <div style={{textAlign:'center',color:'#1E3530',padding:'30px 0',fontSize:12,letterSpacing:'0.1em'}}>
        ...
        filtered.map(p => {
          ...
          return (
            <div key={p.id} style={{background:'#0E1210',border:`1px solid ${p.status==='pending'?'#D97706':'#1E3530'}`,borderRadius:8,padding:'12px 14px',marginBottom:8}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12,flexWrap:'wrap'}}>

                {/* Info */}
                <div style={{minWidth:0}}>
                  <div style={{fontWeight:700,color:'#D4EDE8',fontSize:14,marginBottom:3}}>
                    {p.full_name || '(sin nombre)'}
                  </div>
                  <div style={{fontSize:11,color:'#4A7A70',marginBottom:6}}>{p.email}</div>
                  ...
                  <div style={{display:'flex',gap:4}}>
                    {TRACKS.map(t => (
                      <button key={t} onClick={()=>handleUpdate(p.id,{track:t})}
                        style={{
                          ...
                          border: `1px solid ${p.track===t ? '#BE185D' : '#2A4A44'}`,
                          color: p.track===t ? '#F9A8D4' : '#3A6058',
                        }}>
                        {t}
                      </button>
                    ))}
                    {p.track && (
                      <button onClick={()=>handleUpdate(p.id,{track:null})}
                        style={{padding:'4px 8px',fontSize:10,cursor:'pointer',borderRadius:5,background:'none',border:'1px solid #2A1220',color:'#2A4A44'}}>
                        ✕
                      </button>
                    )}
                  </div>
                  ...
                  <div style={{display:'flex',gap:4}}>
                    ...
                    {p.status !== 'inactive' && (
                      <button onClick={()=>handleUpdate(p.id,{status:'inactive'})}
                        style={{flex:1,padding:'6px 10px',background:'none',border:'1px solid #3D1A2E',borderRadius:6,color:'#4A7A70',cursor:'pointer',fontSize:11}}>
                        Desactivar
                      </button>
                    )}
