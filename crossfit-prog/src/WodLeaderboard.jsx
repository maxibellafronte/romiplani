...
export default function WodLeaderboard({ blockId, resultType, userId, userName, isAdmin }) {
  ...
  return (
    <div style={{ marginTop: 12, background: '#0E0A0D', border: '1px solid #3D1A2E', borderRadius: 10, overflow: 'hidden' }}>

      {/* Header */}
      <div style={{ padding: '10px 14px', background: '#1A0E16', borderBottom: '1px solid #2A1220', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 11, fontWeight: 800, color: ACCENT, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          🏆 Leaderboard
        </span>
        <span style={{ fontSize: 10, color: '#6B3A55', fontWeight: 700, letterSpacing: '0.08em' }}>{typeLabel}</span>
      </div>
        ...
        <div style={{ padding: '12px 14px', borderBottom: '1px solid #1E0A17' }}>
          {myResult && !editMode ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
              <div>
                <span style={{ fontSize: 11, color: '#6B3A55', display: 'block', marginBottom: 3 }}>Tu resultado</span>
                <span style={{ fontSize: 16, fontWeight: 800, color: ACCENT }}>{fmtResult(myResult, resultType)}</span>
              </div>
              <button onClick={() => setEditMode(true)}
                style={{ background: 'none', border: '1px solid #3D1A2E', borderRadius: 6, color: '#6B3A55', cursor: 'pointer', fontSize: 11, padding: '5px 10px' }}>
                Editar
              </button>
            </div>
          ) : (
            <div>
              <span style={{ fontSize: 11, color: '#6B3A55', display: 'block', marginBottom: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input
                  ...
                  placeholder={ph}
                  style={{ flex: 1, background: '#110810', border: '1px solid #3D1A2E', borderRadius: 7, color: '#F5D0E8', padding: '9px 12px', fontSize: 14, outline: 'none' }}
                />
      ...
      {/* Tabla */}
      {loading ? (
        <div style={{ padding: '20px', textAlign: 'center', color: '#2A1220', fontSize: 11, letterSpacing: '0.1em' }}>CARGANDO...</div>
      ) : results.length === 0 ? (
        <div style={{ padding: '20px', textAlign: 'center', color: '#2A1220', fontSize: 11, letterSpacing: '0.1em' }}>— SIN RESULTADOS AÚN —</div>
      ) : (
        <div>
          {results.map((r, i) => {
            ...
            return (
              <div key={r.id} style={{
                ...
                background: isMe ? ACCENT + '10' : i % 2 === 0 ? '#0E0A0D' : '#110810',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ fontSize: 13, fontWeight: isMe ? 700 : 500, color: isMe ? ACCENT : '#D0D0D0', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {r.user_name || 'Atleta'} {isMe && <span style={{ fontSize: 10, color: '#6B3A55' }}>(vos)</span>}
                  </span>
                </div>
                ...
                {isAdmin && (
                  <button onClick={() => handleDelete(r.user_id)}
                    style={{ background: 'none', border: 'none', color: '#3D1A2E', cursor: 'pointer', fontSize: 14, padding: '0 4px', flexShrink: 0 }}>
                    ×
                  </button>
                )}
              </div>
