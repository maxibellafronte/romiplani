...
function rankColor(r) {
  if (r === 1) return '#C8912E'
  if (r === 2) return '#8A9AA8'
  if (r === 3) return '#A9702E'
  return '#5A7286'
}
...
function RankBadge({ rank }) {
  ...
  return (
    <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#C4CDD4', color: '#7A8FA0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700 }}>
      {rank}
    </div>
  )
}
...
function ExpandableNote({ note }) {
  ...
  return (
    <span
      ...
      style={{
        fontSize: 11, color: '#7A8FA0', display: 'block',
...
export default function WodLeaderboard({ blockId, resultType, userId, userName, isAdmin }) {
  ...
  const handleSubmit = async () => {
    ...
    if (resultType === 'tiempo') {
      ...
      if (secs > 59) {
        setError('Los segundos deben estar entre 00 y 59')
        return
      }
    }
    setSubmitting(true)
    ...
    try {
      ...
      // Si no devolvió fila, el guardado fue rechazado (RLS)
      if (!saveError && !savedRow) {
        throw new Error('El servidor rechazó el guardado. Revisá los permisos de la tabla.')
      }
  ...
  return (
    <div style={{ marginTop: 12, background: '#EEF2F0', border: '1px solid #AEB9C0', borderRadius: 10, overflow: 'hidden' }}>

      {/* Header */}
      <div style={{ padding: '10px 14px', background: '#E2E8EA', borderBottom: '1px solid #C4CDD4', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 11, fontWeight: 800, color: ACCENT, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
          🏆 Leaderboard
        </span>
        <span style={{ fontSize: 10, color: '#7A8FA0', fontWeight: 700, letterSpacing: '0.08em' }}>{typeLabel}</span>
      </div>

      {/* Carga de resultado — solo atletas */}
      {!isAdmin && (
        <div style={{ padding: '12px 14px', borderBottom: '1px solid #C4CDD4' }}>
          {myResult && !editMode ? (
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
              <div>
                <span style={{ fontSize: 11, color: '#7A8FA0', display: 'block', marginBottom: 3 }}>Tu resultado</span>
            ...
            <div>
              <span style={{ fontSize: 11, color: '#7A8FA0', display: 'block', marginBottom: 8 }}>
              ...
              <div style={{ marginTop: 10 }}>
                <span style={{ fontSize: 11, color: '#7A8FA0', display: 'block', marginBottom: 6 }}>
                  Notas <span style={{ fontSize: 10, color: '#AEB9C0' }}>(opcional)</span>
                </span>
                <textarea
                  value={notesVal}
      ...
      {/* Tabla de resultados */}
      {loading ? (
        <div style={{ padding: '20px', textAlign: 'center', color: '#AEB9C0', fontSize: 11, letterSpacing: '0.1em' }}>CARGANDO...</div>
      ) : results.length === 0 ? (
        <div style={{ padding: '20px', textAlign: 'center', color: '#C4CDD4', fontSize: 11, letterSpacing: '0.1em' }}>— SIN RESULTADOS AÚN —</div>
      ) : (
        <div>
          {results.map((r, i) => {
            const isMe = r.user_id === userId
            const rc = rankColor(r.rank)
            const isBeingEdited = isAdmin && adminEditId === r.user_id
            ...
            if (isBeingEdited) {
              return (
                <div key={r.id} style={{
                  padding: '10px 14px',
                  background: '#F0E6D2',
                  borderBottom: i < results.length - 1 ? '1px solid #C4CDD4' : 'none',
                }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: '#1F3A4A', marginBottom: 8 }}>
                    {r.user_name || 'Atleta'}
                  </div>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                    ...
                    <button onClick={() => handleAdminSave(r.user_id)} disabled={adminSaving}
                      style={{ padding: '7px 14px', background: '#31708E', border: 'none', borderRadius: 6, color: 'white', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>
                      {adminSaving ? '...' : '✓'}
                    </button>
                    <button onClick={cancelAdminEdit}
                      style={{ padding: '7px 10px', background: 'none', border: '1px solid #AEB9C0', borderRadius: 6, color: '#7A8FA0', cursor: 'pointer', fontSize: 12 }}>
                      ✕
                    </button>
                  </div>
                  <textarea
                    ...
                    rows={2}
                    placeholder="Notas..."
                    style={{ width: '100%', background: '#EEF2F0', border: '1px solid #AEB9C0', borderRadius: 6, color: '#1F3A4A', padding: '7px 10px', fontSize: 12, outline: 'none', resize: 'none', fontFamily: 'inherit' }}
                  />
                </div>
              )
            }
            ...
            return (
              <div key={r.id} style={{
                ...
                background: isMe ? ACCENT + '12' : i % 2 === 0 ? '#EEF2F0' : '#E4EAEC',
                borderBottom: i < results.length - 1 ? '1px solid #C4CDD4' : 'none',
              }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <span style={{ fontSize: 13, fontWeight: isMe ? 700 : 500, color: isMe ? ACCENT : '#1F3A4A', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {r.user_name || 'Atleta'} {isMe && <span style={{ fontSize: 10, color: '#7A8FA0' }}>(vos)</span>}
                  </span>
                ...
                {isAdmin && (
                  <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
                    <button onClick={() => startAdminEdit(r)}
                      style={{ background: 'none', border: 'none', color: '#7A8FA0', cursor: 'pointer', fontSize: 13, padding: '2px 5px', lineHeight: 1 }}>
                      ✏
                    </button>
                    <button onClick={() => handleDelete(r.user_id)}
                      style={{ background: 'none', border: 'none', color: '#AEB9C0', cursor: 'pointer', fontSize: 16, padding: '0 4px', lineHeight: 1 }}>
                      ×
                    </button>
                  </div>
                )}
