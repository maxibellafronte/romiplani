...
function RankBadge({ rank }) {
  ...
  return (
    <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#DDE5EC', color: '#7A8FA0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700 }}>
      {rank}
    </div>
  )
}
...
export default function WodLeaderboard({ blockId, resultType, userId, userName, isAdmin }) {
  ...
  return (
    <div style={{ marginTop: 12, background: '#FFFFFF', border: '1px solid #C0CDD8', borderRadius: 10, overflow: 'hidden' }}>

      {/* Header */}
      <div style={{ padding: '10px 14px', background: '#F7F9FB', borderBottom: '1px solid #DDE5EC', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {!isAdmin && (
        <div style={{ padding: '12px 14px', borderBottom: '1px solid #DDE5EC' }}>
          {myResult && !editMode ? (
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
              ...
              <button
                ...
                style={{ background: 'none', border: '1px solid #C0CDD8', borderRadius: 6, color: '#7A8FA0', cursor: 'pointer', fontSize: 11, padding: '5px 10px', flexShrink: 0 }}>
                Editar
              </button>
            </div>
          ) : (
            <div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {editMode && (
                  <button
                    ...
                    style={{ padding: '9px 12px', background: 'none', border: '1px solid #C0CDD8', borderRadius: 7, color: '#7A8FA0', cursor: 'pointer', fontSize: 13 }}>
                    ✕
                  </button>
                )}
              </div>
              ...
              <div style={{ marginTop: 10 }}>
                <span style={{ fontSize: 11, color: '#7A8FA0', display: 'block', marginBottom: 6 }}>
                  Notas <span style={{ fontSize: 10, color: '#C0CDD8' }}>(opcional)</span>
                </span>
                <textarea
                  value={notesVal}
              ...
              {error && (
                <div style={{ marginTop: 8, padding: '8px 12px', background: '#FBEDED', border: '1px solid #C25454', borderRadius: 6 }}>
      ...
      {/* Tabla de resultados */}
      {loading ? (
        <div style={{ padding: '20px', textAlign: 'center', color: '#C0CDD8', fontSize: 11, letterSpacing: '0.1em' }}>CARGANDO...</div>
      ) : results.length === 0 ? (
        <div style={{ padding: '20px', textAlign: 'center', color: '#E4EAF0', fontSize: 11, letterSpacing: '0.1em' }}>— SIN RESULTADOS AÚN —</div>
      ) : (
        <div>
          {results.map((r, i) => {
            ...
            return (
              <div key={r.id} style={{
                ...
                background: isMe ? ACCENT + '12' : i % 2 === 0 ? '#FFFFFF' : '#FAFCFD',
                borderBottom: i < results.length - 1 ? '1px solid #DDE5EC' : 'none',
              }}>
                ...
                {isAdmin && (
                  <button onClick={() => handleDelete(r.user_id)}
                    style={{ background: 'none', border: 'none', color: '#C0CDD8', cursor: 'pointer', fontSize: 16, padding: '0 4px', flexShrink: 0, lineHeight: 1 }}>
                    ×
                  </button>
                )}
              </div>
