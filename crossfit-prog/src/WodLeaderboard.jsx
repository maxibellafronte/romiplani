          ...
          {myResult && !editMode ? (
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
              <div>
                ...
                {myNotes && <p style={{ fontSize: 12, color: '#5A8A80', marginTop: 4, fontStyle: 'italic' }}>{myNotes}</p>}
              </div>
              <button onClick={() => { setEditMode(true); setNotesVal(myNotes); }}
            ...
            <div>
              ...
              <div style={{ marginTop: 10 }}>
                <span style={{ fontSize: 11, color: '#4A7A70', display: 'block', marginBottom: 6 }}>Notas <span style={{ fontSize: 10, color: '#2A4A44' }}>(opcional)</span></span>
                <textarea
                  value={notesVal}
                  onChange={e => setNotesVal(e.target.value)}
                  placeholder="¿Cómo fue? Ej: Rx, escalé las dominadas, tiempo límite..."
                  rows={2}
                  style={{ display: 'block', width: '100%', background: '#0E1210', border: '1px solid #2A4A44', borderRadius: 7, color: '#D4EDE8', padding: '9px 12px', fontSize: 13, outline: 'none', resize: 'none', fontFamily: 'inherit', lineHeight: 1.5 }}
                />
              </div>
            ...
            return (
              <div key={r.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  {r.notes && <span style={{ fontSize: 11, color: '#4A7A70', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontStyle: 'italic' }}>{r.notes}</span>}
                </div>
